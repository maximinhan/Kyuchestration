...
**태그의 앞자리는 1 이상이어야 한다.** 맥의 jpackage 가 앱 버전을 CFBundleVersion 규약으로
검사해서 첫 숫자가 0 이면 거절하기 때문이다 — 애플의 규약에서 `0.x` 는 아직 내놓은 적 없는
빌드라는 뜻이다. 워크플로 첫 잡이 이것을 15 초 만에 확인하고 끊으므로, 잘못된 태그가 십 분을
태운 끝에 죽는 일은 없다.
