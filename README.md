...
desktop/         # 데스크톱 앱 (Kotlin + Compose Multiplatform, 독립 Gradle 빌드)
    ...
    workdir/kyucli/  # kyu 를 자식 프로세스로 부르는 어댑터 (JSON 계약이 닿는 유일한 곳)
    ...
    kyu/         # kyu 의 자리에 대한 지식 — 실행 파일 경로와 계획 파일 경로
```
...
4. **레포를 넣는다** — 지금은 그 디렉토리에서 [`kyu clone`](#kyu-clone) 으로 고르거나 직접
   `git clone` 한다. 클론도 앱 안으로 들어올 예정이다.
