...
확인되지 않았다([app-owned-sessions-design.md](app-owned-sessions-design.md) 부록 A). 그 셋을
치우는 것은 별도 작업이다(그 문서 5.7 — "윈도우는 별도 작업이고 이 문서의 네 단계에 들어 있지
않다").
...
| **대화 자리** (나머지) | 고른 세션과의 [대화](#앱-안의-챗). 원시 [터미널](#임베디드-터미널)은 머리말에서 고른 동안만 |

좌우로 나눈 것은 [챗 UI 설계](chat-ui-design.md) 6.2 의 셸이다. 위아래였던 이유는 터미널이 폭을
먹기 때문이었는데(세션 안의 화면은 80 칸 기준이다), 그 자리에 선 대화는 폭에 맞춰 흐른다.
...
**챗 머리말의 `터미널로 보기` 로 열린다.** 원시 터미널은 챗이 아직 덮지 못한 것들
(`/login` 의 브라우저 왕복 · `/mcp` 의 서버 승인 · 플랜 모드의 승인 화면)을 위해 남아 있다.
누르면 **그 세션의 챗이 끝나고 같은 대화가 터미널로 다시 열린다** — 한 세션은 한 종류이고,
대화 하나를 두 화면이 동시에 열면 `claude` 가 거절한다.
...
끼운다. 돌아오면 두고 온 화면이 그대로 있다 — 그 화면을 쥐고 있는 것이 앱 안의 터미널 위젯이라
다시 그릴 필요가 없다.
...
대화를 열어 주지 않는다** — 끝난 화면을 그대로 두고 머리말에 그 사실을 적는다. 이유는 화면
