...
git 상태 칩(`DIRTY` 주황 · `AHEAD` 파랑 · `IDLE` 회색), 계획에서 고른 지금 볼 작업
...
**앱 세션은 앱과 생사를 같이하고, 대화는 이어진다.** 잃는 것은 진행 중이던 턴이고, 잃지 않는
것은 이미 한 일과 대화 전체다. `kyu` 가 세션마다 대화 ID 를 정해 워크디렉토리의
`.coord/conversations.json` 에 적어 두기 때문에, 앱을 껐다 켜고 같은 카드를 누르면 `claude
--resume` 으로 **그 대화가 이어진다.** 전사가 사라진 경우(`~/.claude` 를 정리했거나 다른
머신에서 연 경우)에는 세션이 곧바로 끝나고 터미널에 그 이유가 남는다 — 그때 자동으로 새 대화를
여는 복구는 설계의 3 단계다.
