...
**터미널을 한 번도 열지 않고 0 에서 작업을 시작할 수 있다** — 만들기 · 클론 · 세션 진입이 모두
창 안에 있다. 다만 `kyu` 바이너리는 여전히 `PATH` 에 있어야 한다. 앱이 CLI 를 감추는 것이지
대신하는 것이 아니고, 그 셋을 실제로 하는 것은 전부 엔진이다.
