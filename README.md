...
2. **설치 패키지로 받았다면 이 걸음이 없다** — 엔진이 앱 안에 들어 있어 곧바로 다음 화면이다
   ([앱은 엔진을 들고 온다](#앱은-엔진을-들고-온다)). 소스에서 띄웠고 `kyu` 도 없으면 그때
   **[엔진 설치]** 가 뜨고, 앱이 최신 릴리스에서 받아 `kyu version` 으로 확인한 뒤 넘어간다.
...
#### 동봉이 없는 실행 — [엔진 설치] 는 그대로 있다

소스에서 띄우면(`./gradlew run`) 동봉이 없다. 그때는 예전과 같다: 엔진이 없으면 첫 화면이 설치
화면이고, 버튼 하나가 최신 릴리스에서 이 머신에 맞는 바이너리를 받아 놓는다. 받는 방식은
[install.sh](#a-설치-스크립트-추천) 와 같다 — 최신 릴리스 조회도 자산 내려받기도 **인증하지
...
소스에서 엔진까지 함께 만들려면 `./gradlew buildBundledEngine` 이 옆 Go 모듈에서 이 머신용 `kyu`
를 만들어 동봉 자리에 놓는다. 설치 패키지를 만드는 `packageDeb` · `packageDmg` 는 그것이 없으면
안내와 함께 멈춘다 — 엔진이 빠져도 패키징은 끝까지 성공하므로, 받아서 설치해 보기 전에는 아무도
모르기 때문이다.
