...
3. **워크디렉토리 열기** 로 디렉토리를 고른다 — 고른 그 자리가 곧 워크디렉토리다. 계획 파일이
   없으면 그 자리에 만들어 두고 곧바로 대시보드로 넘어간다. 빈 자리에서 시작하려면 선택창의
   **새 폴더** 버튼으로 만들어 고른다.
...
**계획 파일이 도중에 사라지면** 목록 위에 안내 한 줄과 **초기화** 버튼이 붙는다. 열 때 이미
초기화하므로 여기까지 오는 길은 앱이 도는 동안 밖에서 `.coord` 를 지운 경우다. 막는 것이 아니라
알리는 것이다 — [`kyu list`](#kyu-list-path) 는 계획 없이도 답하므로 카드와 세션은 그대로 쓸 수
...
디렉토리를 고르면 그 안의 레포를 카드로 보여준다. 카드 한 장은 [`kyu list`](#kyu-list-path) 가
답한 사실 위에 **앱 자신이 아는 사실을 얹은 것**이다 — 세션 칩(`앱 세션` 초록 · `CLI 세션` 보라),
`이어갈 대화` 칩(청록), git 상태 칩(`DIRTY` 주황 · `AHEAD` 파랑 · `IDLE` 회색), 계획에서 고른 지금 볼 작업
...
가리키던 기록이 새 것으로 바뀌어 **되돌릴 수 없다**. 앱 세션이 이미 도는 카드에는 둘 다 뜨지
...
**카드를 옮겨도 앞 세션은 계속 돈다.** 앱은 워크디렉토리 안의 세션을 여럿 보유하고 화면만 갈아
끼운다. 돌아오면 두고 온 화면이 그대로 있다 — 그 화면을 쥐고 있는 것이 앱 안의 터미널 위젯이라
다시 그릴 필요가 없다.

**끝내는 것과 옮기는 것은 다르다.** 터미널 머리말의 **세션 끝내기** 는 진짜로 그 세션을 끝낸다
...
두기 때문이다. 앱을 껐다 켜서 그 워크디렉토리를 다시 열면 이어갈 것이 있는 카드에 **`이어갈
대화` 칩**이 붙어 있고, 그 카드를 누르면 `claude --resume` 으로 **그 대화가 이어진다.** 확인
