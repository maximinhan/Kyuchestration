...
| **대화 자리** (나머지) | 고른 세션. 지금은 [임베디드 터미널](#임베디드-터미널)이 그대로 뜬다 |

좌우로 나눈 것은 [챗 UI 설계](chat-ui-design.md) 6.2 의 셸이다. 지금까지 위아래였던 이유는
터미널이 폭을 먹기 때문이었는데(세션 안의 화면은 80 칸 기준이다), 그 자리에 들어올 대화는 폭에
맞춰 흐른다. **이 걸음에서 바뀐 것은 자리이고 화면 모델은 그대로다** — 레일에서 세션을 고르면
지금과 똑같이 터미널이 열린다.
...
**왼쪽 패널에서 줄을 누르면 오른쪽 대화 자리에 그 세션이 열린다.** 맨 위의 조율 블록을 누르면
메인 세션이다. 한 번 누르는 것으로 작업이 시작된다.
