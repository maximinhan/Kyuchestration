...
//	  ],
//	  "mainSessionRestartNeeded": true               // 떠 있는 메인 세션이 새 레포를 보지 못한다
//	}
//
...
type cloneJSONDocument struct {
	...
	// MainSessionRestartNeeded 는 떠 있는 메인 세션이 방금 클론한 레포를 보지 못하는지다.
	//
	// 사람용 모드에서 stderr 로 나가던 안내가 여기 들어온다. 읽는 쪽은 stdout 하나만 보므로,
	// 그 안내를 스트림에 남겨두면 앱은 사용자에게 "왜 새 레포가 세션에서 안 보이는지" 를
	// 설명할 방법이 없다.
...
func writeCloneResultsAsJSON(out io.Writer, attempts []cloneAttempt, mainSessionRestartNeeded bool) error {
	...
	return writeJSONDocument(out, cloneJSONDocument{
		...
		Results:                  results,
		MainSessionRestartNeeded: mainSessionRestartNeeded,
	})
}
