...
**결론: 지웠다 (2026-09-09 · PR 62). 무엇이 사라졌는지는 7.4 에 적는다.**
...
| 중단 | ✅ 3.8 |
| 대화 이어가기 | ✅ 3.9 |
| `/login` · OAuth 흐름 | **❌ 챗에 길이 없다** — 4 단계에서 쟀다. 아래 7.1 |
| `/mcp` 의 서버 승인 프롬프트 | **✅ 관문 자체가 없다** — 프로젝트 `.mcp.json` 이 묻지 않고 붙는다. 다만 인증이 필요한 서버는 위 칸과 같은 자리에서 막힌다. 아래 7.1 |
| 플랜 모드의 승인 화면 | **✅ 막지 않는다** — 모드는 그대로 돌고, 승인은 카드가 아니라 대화로 온다. 아래 7.1 |
| `AskUserQuestion` | 도구 목록에 있다. 카드가 될 것이다 |

### 7.1 ❓ 셋을 쟀다 (2026-09-04 · `claude` 2.1.259)

4 단계가 격리된 스크래치에서 쟀다. **실제 자격 증명은 건드리지 않았다** — 로그인 흐름은 `CLAUDE_CONFIG_DIR` 을 빈 디렉토리로 돌려 "자격 증명이 없는 설치" 를 흉내 냈다.

**(가) `/login` — 챗 모드에 길이 없다.**

| 잰 것 | 받은 것 |
|---|---|
| 챗 모드에 `/login` 을 보낸다 | `assistant`: `"/login isn't available in this environment."` · `result` 는 `success` |
| 자격 증명이 없는 채로 아무 말이나 건다 | `assistant`: `"Not logged in · Please run /login"` → **종료 코드 1 로 프로세스가 끝난다** |
| `init.slash_commands` (49 개) | `login` 이 없다. `terminal_slash_commands` 는 `doctor`·`color` 둘뿐이다 |

**앱이 이 상태를 알아볼 수는 있다.** 세션이 한 줄을 답하고 코드 1 로 끝나므로 화면에 전사와 종료 코드가 남는다. 그러나 **거기서 할 수 있는 일이 없다** — 로그인은 브라우저를 여는 대화형 흐름이고, 챗 모드는 그것을 태우지 못한다.

**(나) `/mcp` 서버 승인 — 관문이 없다.**

격리 레포에 `.mcp.json`(`kyu mcp serve`)을 두고 챗 모드로 띄웠다.

| 잰 것 | 받은 것 |
|---|---|
| 프로젝트 `.mcp.json` 의 서버 | 승인을 묻지 않고 붙었다 — `init.mcp_servers` 에 `{"name":"probe-kyu","status":"connected"}` |
| `/mcp` | 요약 한 줄. `"4 MCP server(s): 1 connected, 3 not connected, 0 disabled. Use /mcp in the terminal for details."` |
| 인증이 필요한 서버 | `status: "needs-auth"` 로 보고된다. 그 인증을 챗에서 할 길은 없다 — (가)와 같은 자리다 |

orchestration 3.2 의 짐작이 맞았다: **헤드리스는 그 관문을 타지 않는다.** 승인 프롬프트가 없으니 승인이 필요해서 안 붙는 서버도 없다.

**(다) 플랜 모드 — 돌지만 승인이 카드로 오지 않는다.**

`--permission-mode plan` 으로 띄우면 `init.permissionMode` 가 `plan` 으로 온다. 그런데 이 판의 `claude` 는 **`ExitPlanMode` 를 세션의 도구 목록에 두지 않았다**(모델이 `ToolSearch` 로 찾다가 없다고 답했다). 모델은 계획을 `~/.claude/plans/<이름>.md` 에 `Write` 로 적고 **본문 텍스트로** 승인을 청했다.

그래서 승인 카드가 될 도구 호출은 오지 않는다. **막히는 것도 없다** — 사용자가 대화로 답하면 그대로 이어진다. 카드가 되는 날은 그 도구가 세션에 실려 오는 날이고, 그때는 다른 도구와 같은 길로 온다.

### 7.2 삭제 조건이 채워졌다 — 앱이 로그인을 직접 태운다

셋 중 둘은 챗에서 되거나 막지 않는다. 남은 하나가 **OAuth 류 흐름**이다 — 계정 로그인과 MCP 서버 인증이 같은 자리에서 막힌다.
...
| **터미널 코드를 삭제한다** | 9 절 7 단계 ✅ (2026-09-09 · PR 62 — 7.4) |
...
**앱이 태우고 엔진이 맡는다.** 둘로 갈리는 자리를 적어둔다.

| 무엇 | 어디 | 왜 |
|---|---|---|
| 자격 증명이 있는지 묻기 · 브라우저 로그인 · 토큰 검증 | **앱** (`claudeauth/process/`) | 엔진에는 `claude` 에게 물을 클라이언트가 없다. 그리고 이것은 "세션이 무엇을 띄우는가" 의 물음이 아니라 앱이 자기 온보딩을 위해 하는 물음이라 원칙 11 의 자리가 아니다 |
| 토큰 맡아 두기 | **엔진** (`kyu claude-auth`) | 이 도구의 비밀이 사는 자리가 `internal/secretstore` 하나다. 앱이 저장 방식을 다시 알기 시작하면 GitHub 토큰과 claude 토큰이 다른 규칙으로 저장된다 |
| 세션에 실을 환경변수 이름 | **앱** (`platform/ClaudeAuthTokenEnvironment.kt`) | **원칙 11 의 예외다.** 엔진의 답에 실으면 `kyu session-command --json` 을 친 사람의 화면에 토큰이 통째로 찍히고, 그 값은 폐기 전까지 유효하다. 그리고 앱은 어차피 감지할 때 같은 이름을 써야 한다 — 엔진에 두면 이름이 두 곳에 놓인다 |

**(나)가 유력하다고 적었던 앞의 판단은 틀렸다.** 그 판단은 "로그인은 브라우저를 여는 대화형 흐름이고 챗 모드는 그것을 태우지 못한다"(7.1)에서 곧바로 "그러니 PTY 하나가 남아야 한다" 로 건너뛰었다. 건너뛴 자리에 잰 것이 없었다 — 7.1 이 잰 것은 `-p` 스트림 **안**이고, `claude` 가 그 밖에서 무엇을 주는지는 재지 않았다. 7.3 이 그것을 쟀고 답은 **PTY 가 필요 없다** 였다: `claude auth login` 은 파이프에 물려도 브라우저 URL 을 stdout 으로 내고 코드를 stdin 한 줄로 받는다. 그러면 (나)가 남기려던 "PTY 하나를 띄워 사용자가 `/login` 을 치게 하는 자리" 는 아예 생기지 않는다.

**그래서 7 단계의 조건이 채워졌다.** 로그인이 필요한 상태를 앱이 다루는 자리가 터미널 밖에 섰으므로, JediTerm·pty4j 와 `terminal/pty/` 를 지우는 PR 이 열린다. 그 삭제는 이 단계에서 하지 않는다 — 삭제는 그 자체로 한 줄기이고, 인증과 같은 PR 에 두면 "인증이 안 되는 것" 과 "터미널을 지워서 안 되는 것" 이 한 리뷰에 섞인다.

**MCP 서버 인증은 이 결정으로 풀리지 않는다.** 계정 로그인과 같은 자리에서 막힌다고 적었지만 같은 명령으로 풀리지는 않는다 — `claude auth login` 은 앤트로픽 계정에 대한 것이고, `status: "needs-auth"` 인 서버(7.1 의 나)는 그 서버의 인증을 따로 요구한다. 이 설계는 그것을 다루지 않는다. 실제로 그런 서버를 붙이는 날에 다시 연다.
...
### 7.3 ❓ 로그인을 쟀다 (2026-09-08 · `claude` 2.1.259)

7.1 과 같은 자세다 — `CLAUDE_CONFIG_DIR` 을 빈 임시 디렉토리로 돌려 **자격 증명이 없는 설치**를 재현했고, 실제 자격 증명은 읽지도 고치지도 않았다. 실계정으로 로그인을 끝내는 걸음은 사람의 몫이라 재지 않았다 — 잰 것은 흐름의 기계적인 부분(URL 이 나오는가 · 입력을 기다리는가 · 환경변수를 알아보는가)까지다.

| 인증 수단 | 앱이 구동할 수 있나 | 브라우저 URL 을 어떻게 얻나 | 완료를 어떻게 아나 | 유효한지 어떻게 아나 | 저장 자리 |
|---|---|---|---|---|---|
| `claude auth login` | ✅ **파이프로 된다 — PTY 가 필요 없다** | stdout 한 줄: `If the browser didn't open, visit: <URL>` | 코드를 stdin 에 한 줄 쓰면 종료 코드로 갈린다 (0 · 1) | 코드 교환이 곧 검증이다 | **`claude` 자신** — 앱은 저장할 것이 없다 |
| `claude auth status --json` | ✅ | — | — | **보지 않는다.** 가짜 토큰에도 `loggedIn: true` 다 | — |
| `claude setup-token` | ❌ 파이프에 물리면 **아무것도 내지 않고 멎는다** | — | — | — | 사람이 자기 터미널에서 |
| `CLAUDE_CODE_OAUTH_TOKEN` | ✅ 환경변수 하나 | — | — | `claude -p` 한 턴이 가른다 | 앱 (`kyu claude-auth`) |
| `ANTHROPIC_API_KEY` | ✅ 환경변수 하나 | — | — | 같은 방식 | **v1 범위 밖** |

**(가) `claude auth login` — 파이프 위에서 그대로 돈다.**

| 잰 것 | 받은 것 |
|---|---|
| stdin 을 `/dev/null` 로 두고 띄운다 | stdout 에 `Opening browser to sign in…` 과 URL 한 줄. 그 뒤 `Paste code here if prompted > ` 를 찍고 **멈춘 채로 기다린다**(25 초 한도 초과) |
| 코드 한 줄을 파이프로 넣는다 (가짜 코드) | stderr 에 `Login failed: Request failed with status code 400` · **종료 코드 1** |
| PTY 에 물려 같은 것을 잰다 | 같다 — 문구도 순서도 같고, 색 코드만 더 붙는다 |

**URL 이 OSC 8 하이퍼링크로 감싸여 온다.** 바이트로는 `ESC ] 8 ; ; <URL> BEL <URL> ESC ] 8 ; ; BEL` 이라 **같은 URL 이 두 번** 실려 있다 — 터미널이 링크로 그릴 대상 하나와, 링크를 모르는 터미널에 보일 글자 하나다. 파싱하는 쪽은 OSC 시퀀스를 걷어낸 뒤 첫 `https://` 를 잡아야 한다. 그러지 않으면 URL 이 두 번 이어붙은 문자열을 얻는다.

**되돌아오는 자리가 로컬이 아니다.** URL 의 `redirect_uri` 가 `https://platform.claude.com/oauth/code/callback` 이라, 브라우저가 앱의 포트로 돌아오지 않는다. **완료를 감지할 길은 사용자가 붙여넣는 코드 하나뿐이다** — 앱이 열어 둘 콜백 서버도, 폴링할 자리도 없다. 화면이 코드 입력 칸을 반드시 가져야 하는 근거가 이것이다.

**(나) `claude auth status --json` — 있음과 없음만 답한다.**

| 잰 것 | 받은 것 |
|---|---|
| 빈 `CLAUDE_CONFIG_DIR` | `{"loggedIn": false, "authMethod": "none", …}` · 종료 코드 1 |
| 같은 자리 + `CLAUDE_CODE_OAUTH_TOKEN=<가짜>` | `{"loggedIn": true, "authMethod": "oauth_token", …}` · 종료 코드 0 |
| 같은 자리 + `ANTHROPIC_API_KEY=<가짜>` | `{"loggedIn": true, "authMethod": "api_key", "apiKeySource": "ANTHROPIC_API_KEY", …}` · 종료 코드 0 |

**가짜 토큰에도 `true` 라고 답한다.** 이 명령이 보는 것은 자격 증명이 **있는가**이지 그것이 **통하는가**가 아니다. 그래서 감지에는 쓰고 검증에는 쓰지 않는다 — 검증은 (라)가 한다.

**(다) `claude setup-token` — 앱이 태울 수 없다.**

stdin 을 파이프로 물려 띄우면 stdout·stderr 에 한 글자도 내지 않고 20 초 한도까지 멎어 있었다. 대화형 전용이다. 그래서 이것은 **앱의 흐름이 아니라 사용자의 흐름**이다: 이미 로그인된 머신에서 사람이 자기 터미널에 쳐서 장기 토큰을 얻고, 그 토큰을 새 머신의 앱에 붙여넣는다. 토큰 폴백이 있는 이유가 그것이다.

**(라) 토큰이 통하는지는 `claude -p` 한 턴이 가른다.**

| 잰 것 | `result` 의 값 | 종료 코드 |
|---|---|---|
| 자격 증명 없음 | `result: "Not logged in · Please run /login"` · `api_error_status: null` | 1 |
| `CLAUDE_CODE_OAUTH_TOKEN=<가짜>` | `result: "Failed to authenticate. API Error: 401 OAuth access token is invalid."` · `api_error_status: 401` | 1 |

**둘이 갈린다는 것이 중요하다.** 앞은 "아무것도 없다", 뒤는 "있는데 거절당했다" 이고, 사용자가 할 일이 다르다. `is_error` 와 `api_error_status` 가 그 구분을 그대로 준다.

검증에 쓰는 명령은 최소로 좁혔다 — `-p --output-format json --setting-sources "" --strict-mcp-config --tools "" --no-session-persistence`. 사용자 설정·MCP 서버·도구를 다 걷어내고 전사도 남기지 않는다. 그러지 않으면 토큰 하나를 확인하려고 그 머신의 훅과 MCP 서버가 함께 도는데, 그것은 이 물음의 답과 아무 상관이 없다. 위 표의 두 값은 이 플래그 조합에서도 같았다.
...
- **`TerminalSessionFailure` 라는 이름.** 이제 담는 것은 챗 세션의 진입 실패뿐이라 `SessionEntryFailure` 가 맞다. 이 PR 에서 바꾸지 않은 것은 그 개명이 `terminal/chat/` 의 파일 열 개를 건드리기 때문이다 — 이 PR 을 쓰는 동안 그 자리를 6 단계가 쥐고 있었고, 삭제와 개명을 한 PR 에 섞으면 리뷰가 둘로 갈린다. 개명은 그 자체로 한 줄기다.
...
**7 단계는 2026-09-09 에 끝났다(PR 62).** 무엇이 사라졌고 패키지가 얼마나 줄었는지는 7.4 에
적었다. 6 단계와 나란히 진행했다 — 지운 것이 터미널 쪽이고 6 단계가 만든 것이 `terminal/chat/`
안이라 겹치는 파일이 거의 없었다. 6 단계가 먼저 머지된 뒤 리베이스에서 실제로 부딪친 것은 두
줄이다: 6 단계가 새로 만든 위임 검증이 이 PR 이 지운 `SessionMode` 를 부르고 있었다.
...
1. **`/login`·`/mcp` 승인·플랜 모드가 챗에서 되는가** — **4 단계에서 쟀다. 7.1 이 그 표다.**

   답이 갈렸다. `/mcp` 의 서버 승인은 **관문 자체가 없어** 프로젝트 서버가 그냥 붙고, 플랜 모드는 **돌지만 승인이 카드가 아니라 대화로** 온다 — 둘 다 챗을 막지 않는다. `/login` 만 **길이 없다**: 챗 모드가 `"/login isn't available in this environment."` 라고 답하고, 자격 증명이 없으면 한 줄을 답한 뒤 종료 코드 1 로 끝난다.

   실제 계정을 건드리지 않고 재는 길이 있었다 — `CLAUDE_CONFIG_DIR` 을 빈 디렉토리로 돌리면 "로그인 안 된 설치" 가 그대로 재현된다. 앱에서 직접 해보는 것이 유일한 길이라고 적었던 것은 틀렸다.

   **그 뒤 (2026-09-08): 남은 하나도 해소됐다.** 7.1 이 잰 것은 `-p` 스트림 **안**이었고, `claude` 가 그 밖에서 무엇을 주는지는 재지 않은 채로 "PTY 가 하나 남아야 한다" 로 좁혔다. 7.3 이 그 자리를 쟀다 — `claude auth login` 은 파이프에 물려도 주소를 stdout 으로 내고 코드를 stdin 한 줄로 받는다. **PTY 가 필요 없다.**

   그래서 사용자가 (다) 앱이 직접 태운다를 골랐고, 브라우저가 서지 않는 자리를 위해 토큰 붙여넣기 폴백을 뒀다(7.2). 9 절 6.5 단계가 그것이고, 그 단계가 서면서 **7 단계의 조건이 채워졌다.**

   **이 항목은 닫는다.** 남은 것은 MCP 서버 인증 하나인데, 그것은 이 설계가 다루는 물음이 아니다 — `claude auth login` 은 앤트로픽 계정에 대한 것이고 `status: "needs-auth"` 인 서버는 그 서버의 인증을 따로 요구한다(7.2 의 마지막 문단). 실제로 그런 서버를 붙이는 날에 다시 연다.

2. **컨텍스트가 찰 때 무엇이 오는가** — **3 단계에서 쟀다. 온다.**

   1 000 000 토큰 창을 채우는 것은 프로브의 예산 밖이라 `/compact` 로 압축을 직접 일으켰다(2026-09-04, `claude` 2.1.259, `--setting-sources ""`). 온 순서가 이렇다.

   | 순서 | 줄 | 담긴 것 |
   |---|---|---|
   | 1 | `system/status` | `status: "compacting"` — 시작 |
   | 2 | `system/status` | 30 초 뒤. `status: null` · `compact_result: "success"` — 끝 |
   | 3 | `system/init` | 같은 `session_id` 로 다시 온다(3.1 그대로) |
   | 4 | **`system/compact_boundary`** | `compact_metadata`: `trigger: "manual"` · `pre_tokens: 16805` · `post_tokens: 1794` · `cumulative_dropped_tokens: 15011` · `duration_ms: 30018` · `preserved_segment` · `preserved_messages` |
   | 5 | `user` | 요약 본문. `This session is being continued from a previous conversation that ran out of context…` |
   | 6 | `user` | `<local-command-stdout>Compacted </local-command-stdout>` (`isReplay: true`) |
   | 7 | `result` | `subtype: "success"` · `num_turns: 0` — 모델을 부르지 않았다 |

   **화면이 대화가 짧아진 것을 설명할 재료가 다 있다.** 앞뒤 토큰 수까지 오므로 "여기서 16,805 → 1,794 로 줄었습니다" 를 그대로 적을 수 있다.

   **아직 재지 못한 것 둘을 남긴다.** (가) `trigger` 가 `manual` 인 것만 봤다 — 자동 압축의 값(`auto` 로 짐작되지만 확인 못 함)은 창을 실제로 채워야 갈린다. 필드 이름이 `trigger` 인 것이 갈래가 있다는 뜻이라 자동도 같은 줄로 올 가능성이 크지만, 짐작을 사실로 적지 않는다. (나) **요약이 실린 `user` 이벤트의 `message.content` 가 배열이 아니라 문자열이다.** 지금 어댑터는 그 줄을 `Unrecognized` 로 흘린다 — 잃는 것은 없지만 화면에 아무것도 뜨지 않는다.

   **3 단계 화면에는 아직 그리지 않는다.** 압축 안내를 그리려면 이벤트 갈래 하나와 전사 항목 하나가 늘고, 그것은 이 단계의 완료 확인에 없는 것이다. 잰 값이 다 있으므로 다음 PR 후보로 둔다.
...
8. **큐 깊이를 정확히 그릴 것인가** — **3 단계에서 쟀고, 셀 필요가 없어졌다.**
   ...
   3.11 을 다시 재현했다(긴 답을 시키고 2.0 초 · 2.5 초에 짧은 질문 둘). **보낸 것 3 개, 받은 `result` 2 개** — 3.11 과 같다. 새로 안 것 둘이다.

   - **합쳐진 턴의 되돌아온 `user` 이벤트에 텍스트 블록이 둘 실려 온다.** `[{"text":"빨강…"},{"text":"노랑…"}]` 가 한 메시지에 왔고, 어댑터는 블록마다 하나씩 옮기므로 **말풍선은 셋이 그대로 선다.** 전사는 어긋나지 않는다 — 사용자 말풍선을 앱이 세지 않고 되돌아온 것으로만 그리기 때문이다(원칙 14 · 3.14).
   - **`queued_turn_count` 는 합쳐진 턴에서도 0 이었다.** 큐 깊이를 이 필드로 아는 길은 여전히 없다.

   **그래서 v1 은 아무것도 세지 않는다.** 3 단계는 턴이 도는 동안 입력창을 잠그므로(6.5 와 다르게 정했다 — 중단 버튼이 없는 단계에서 쌓인 말을 되돌릴 길이 없어서다) 큐가 만들어지는 길 자체가 화면에 없다. **잠금을 푸는 4 단계에서 이 항목이 다시 살아난다** — 그때 화면은 "말풍선 하나 = 답 하나" 를 가정하지 않아야 한다.

9. **한글 폰트를 무엇으로 고정할 것인가** — 6.1. 지금은 시스템 기본이라 리눅스와 윈도우가 다르게 보인다. 폰트를 패키지에 담으면 크기가 늘고, 담지 않으면 화면이 머신마다 다르다. **목업 확정 시.**

10. **`init.permissionMode` 를 화면에 보일 것인가** — **3 단계에서 정했다. 보이지 않는다.**

    근거 셋이다.

    - **사용자가 준 적 없는 낱말이다.** 3.6 이 잰 그대로 플래그 이름과 보고 이름이 다르고(`manual` → `default`), 이번 프로브에서도 아무것도 주지 않은 실행이 `permissionMode: "default"` 라고 보고했다. 우리 낱말로 옮기는 길도 있지만, 옮긴 낱말은 `claude` 의 판이 바뀔 때 조용히 어긋난다.
    - **보여도 사용자가 할 수 있는 일이 없다.** 이 값을 정하는 것은 엔진이 조립하는 argv 이고(원칙 11), 앱 화면에 그것을 바꾸는 자리가 없다. 바꿀 수 없는 상태를 머리말에 두면 자리만 차지한다.
    - **정말 필요해지는 자리가 따로 있다.** 관문이 실제로 열리는 것은 5 단계의 승인 카드이고, 그 카드는 도구 이름·인자·cwd 라는 맥락과 함께 뜬다 — "무엇을 물어보는 모드인가" 는 그 자리에서 말하는 편이 정확하다.

    어댑터도 이 값을 싣지 않는다(`SessionDescribed` 에 필드가 없다). 화면이 쓰지 않는 값을 이벤트에 담아 두면, 그것을 그릴 자리가 없는 채로 필드 이름이 굳는다(원칙 4).
...
26. `claude auth login` 은 **파이프 위에서 그대로 돈다** — 브라우저 URL 을 stdout 에 내고, 코드를 stdin 한 줄로 받고, 종료 코드로 성공·실패를 가른다. PTY 가 필요 없다.
27. 그 URL 은 **OSC 8 하이퍼링크로 감싸여 같은 값이 두 번** 실려 온다. 시퀀스를 걷어내지 않고 이어 읽으면 URL 이 두 번 붙은 문자열이 된다.
28. 로그인의 `redirect_uri` 는 **원격**(`https://platform.claude.com/oauth/code/callback`)이다. 로컬 콜백이 없어 **완료를 아는 길은 사용자가 붙여넣는 코드 하나뿐**이다.
29. `claude auth status --json` 은 자격 증명의 **존재만** 답한다 — 가짜 토큰에도 `loggedIn: true` 다.
30. `claude setup-token` 은 **비대화형에서 아무것도 내지 않고 멎는다.** 앱이 태울 수 없다.
31. `claude -p` 는 **"자격 증명이 없다"(`Not logged in · Please run /login`, `api_error_status: null`)와 "거절당했다"(401)를 갈라 답한다.** 토큰 검증의 근거가 이 구분이다.

**문서 조사로 확인한 것** (코드 실행 없음, 3.15)

32. `com.mikepenz:multiplatform-markdown-renderer` 0.45.0 (2026-08-28)이 Compose Desktop JVM·Material 3 모듈·코드 하이라이트 모듈·스트리밍 전용 API 를 갖는다. Apache-2.0.
33. 그 하이라이터 `dev.snipme:highlights:1.1.0` 은 언어 17 개를 enum 이름 완전일치로 매칭한다 — `json`·`yaml`·`sql`·`diff` 가 없고 `bash`·`js`·`py` 같은 별칭이 매칭되지 않는다.

**아직 안 쟀거나 못 잰 것**

34. ~~`/login`·`/mcp` 승인·플랜 모드가 챗 모드에서 어떻게 되는지~~ — **4 단계에서 쟀다(7.1).** `/login` 은 챗 모드에 길이 없고(`"/login isn't available in this environment."`, 자격 증명이 없으면 종료 코드 1), 프로젝트 `.mcp.json` 은 승인 관문 없이 붙고, 플랜 모드는 돌되 `ExitPlanMode` 가 이 판의 도구 목록에 없어 승인이 대화로 온다.
