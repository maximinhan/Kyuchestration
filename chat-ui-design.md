...
| **터미널 코드를 삭제한다** | **로그인이 필요한 상태를 앱이 어떻게 다룰지 정한 뒤** — 9 절 7 단계 |

**7 단계는 아직 열리지 않는다.** 지금 터미널을 지우면 로그아웃된 사용자가 앱 안에서 할 수 있는 일이 없다. 정할 것은 셋 중 하나다 — (가) 터미널을 그대로 둔다 (나) 앱이 그 상태를 알아보고("Not logged in" + 코드 1) **로그인 전용으로만** 터미널을 여는 좁은 길을 남긴다 (다) 앱이 로그인 흐름을 직접 태운다. **(나)가 유력하다**: 남는 코드가 "PTY 하나를 띄워 사용자가 `/login` 을 치게 하는 자리" 로 줄고, 그 자리는 화면 모델이 아니라 설치 화면(EngineInstallationScreen)에 가까운 것이 된다.
...
28. ~~`/login`·`/mcp` 승인·플랜 모드가 챗 모드에서 어떻게 되는지~~ — **4 단계에서 쟀다(7.1).** `/login` 은 챗 모드에 길이 없고(`"/login isn't available in this environment."`, 자격 증명이 없으면 종료 코드 1), 프로젝트 `.mcp.json` 은 승인 관문 없이 붙고, 플랜 모드는 돌되 `ExitPlanMode` 가 이 판의 도구 목록에 없어 승인이 대화로 온다.
