 ...
 *
 * **로깅 프레임워크를 들이지 않는다.** 한동안은 slf4j 가 의존성 트리에 딸려 와 있었지만(JediTerm 과
 * pty4j 가 자기 진단을 그리로 흘렸다) 그 둘이 사라지면서 지금은 그것조차 없다 — 파일로 남기려면
 * 프레임워크 하나를 새로 들여야 한다. 그 대가가 남는 것에 비해 크다:
