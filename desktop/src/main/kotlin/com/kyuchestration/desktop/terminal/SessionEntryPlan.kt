 ...
 * 쉬운 자리도 거기다. 이 계획을 파이프 셋에 무는 것이
 * [com.kyuchestration.desktop.terminal.chat.ChatSessionOpener] 다(chat-ui-design.md 5.3.1).
 *
...
private fun sessionEnvironment(
    ...
    // 어디서 떴는지에 대한 사실이지 앱이 띄운 자식의 사실이 아니고, 그대로 넘기면 claude 가
    // 자기가 tmux 안에 있다고 믿는다(claude --help 에 --tmux 가 있다).
    environment.remove(INSIDE_TMUX_ENV_NAME)

    // 앱이 죽어도 살아남은 자식은 앱 어디에도 보이지 않는다. 파이프에 물린 자식은 부모가 사라진
    // 것을 알지도 못해서(PTY 시절의 SIGHUP 조차 오지 않는다) 이 자리가 더 중요해졌다. 그때 그
    // 프로세스를 찾을 수 있는 유일한 자리가 ps 이고, 이 표식이 거기서 눈에 띈다
    // (`ps eww` 가 환경을 함께 그린다). app-owned-sessions-design.md 8 절 1 번의 (가)다.
