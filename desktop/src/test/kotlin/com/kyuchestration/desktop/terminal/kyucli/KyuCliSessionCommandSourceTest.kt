...
class KyuCliSessionCommandSourceTest {
    ...
    fun `레포 세션은 그 이름을 인자로 받는다`() {
        ...
        KyuCliSessionCommandSource(runner).sessionCommandFor(WORK_DIR_PATH, SessionTarget.Repo("proj-a"))
    ...
    fun `메인 세션은 레포 이름 자리를 비운다`() {
        ...
        KyuCliSessionCommandSource(runner).sessionCommandFor(WORK_DIR_PATH, SessionTarget.Main)
    ...
    fun `kyu 가 거절하면 그 이유를 그대로 올린다`() {
        ...
        val failure = assertFailsWith<TerminalSessionFailure.SessionCommandRefused> {
            KyuCliSessionCommandSource(runner).sessionCommandFor(WORK_DIR_PATH, SessionTarget.Repo("proj-z"))
        }
